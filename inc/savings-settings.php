 ...
 *
 * @return array<string, array{cost: float, time: float}>
 */
...
function hale_dash_disk_block_titles() {
	...
	$skip = ['node_modules', 'vendor', '.git', 'tests', 'test', '__tests__'];
...
function hale_dash_sanitize_block_savings($input) {
	...
	if (isset($input['blocks']) && is_array($input['blocks'])) {
		foreach ($input['blocks'] as $name => $vals) {
			...
			// Only keep blocks that actually carry a saving.
			if ($cost > 0 || $time > 0) {
				$clean['blocks'][$name] = ['cost' => $cost, 'time' => $time];
			}
		}
	}
		...
		<form action="options.php" method="post">
			...
			<table class="widefat striped" id="hale-dash-block-table">
				...
				<?php foreach ($names as $name) :
					...
					?>
					<tr data-search="<?php echo esc_attr(strtolower($title . ' ' . $name . ' ' . implode(' ', $block_sites))); ?>">
